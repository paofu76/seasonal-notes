...
- 切换笔记或新建前自动落盘，避免快速操作导致未保存内容丢失
- 收藏笔记独立筛选
- 分类、标签、归档及分类/标签下拉直筛
- 文本搜索与日期筛选，搜索同时覆盖分类和标签
- 带每日笔记数量和标题预览的月历
- 笔记记录日期可修改，支持补记过去的日记
- 粗体、斜体、下划线
- 清单、可编辑网格表格、图片插入
- 表格右键增删行列、合并和拆分单元格
- 图片粘贴、右键缩放或删除、未使用附件清理
- 完整数据导入导出、每日自动备份和恢复前保护备份
- 编辑区实时显示字数和行数
- 新建、保存、搜索等常用 macOS 快捷键
- 自动记住窗口大小、列表宽度、季节和动态开关
- 本地 JSON 数据保存
...
测试覆盖存储迁移、损坏文件、完整备份、切换前保存、废纸篓恢复、筛选、分类归档、日期、清单与表格关键流程。新增功能将持久化逻辑放在 `storage.py`、界面放在 `ui/`，并增加对应验证。不要提交个人笔记、附件、密钥、虚拟环境和打包产物。
